...
JAVASCRIPT;
	...
	function renderKmlLink($pContent, $pArgv, $parser) {
		$article = isset($pArgv['article']) && $pArgv['article'] ? $pArgv['article'] : $parser->getTitle()->getText();
		$title = Title::newFromText($article);
		$specialTitle = SpecialPage::getTitleFor( 'GoogleMapsKML' );
		return '<a href="'. htmlspecialchars( $specialTitle->getLocalUrl( 'article=' . $title->getPartialURL() ) ) . '">' . $pContent . '</a>';
	}
	...
	static function renderContent($pContent, &$pParser, &$pLocalParser, &$exporter, $o) {
		...
		static function addMarker(&$pExporter, &$pParser, &$pLocalParser, $pLat, $pLon,
			...
			$titleLink = is_null($titleMaybeNonexistent) ? '' : Linker::link($titleMaybeNonexistent);
			if (count($pTabs)) {
